...
/*   Updated: 2023/08/01 18:47:22 by changhyl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_op_syntax(t_tk_list *tk_list)
{
	t_tk	*p;
	t_tk	*q;
...
t_tk_list	*check_syntax(char *str)
{
	...
	{
		syn_err_nnl(tk_list->head->str, tk_list);
		return (NULL);
	}
	if (check_op_syntax(tk_list) == 1)
		return (NULL);
